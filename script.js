...
function strikeTitleKey(key) {
  ...
  const pianoKey = document.querySelectorAll('.piano-keys > i')[Number(key.dataset.key)%14];
...
if ('IntersectionObserver' in window) {
  ...
  document.querySelectorAll('.timeline article,.project,.experience-detail,.education article').forEach(element => entrances.observe(element));
